...
func NewService(cfg *config.Config) (*TokenService, error) {
...
func (s *TokenService) processERC20Tokens() {
	tokens := ERC20Tokens
	distance := uint64(10000)
	toBlock := uint64(0)
	logTransferSig := []byte("Transfer(address,address,uint256)")

	for i := s.cfg.StartBlock; i < s.cfg.EndBlock; i = toBlock + 1 {
